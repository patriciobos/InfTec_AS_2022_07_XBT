...
\clearpage
\subsection{Puente balanceado con JFET}
...
En la FIG. \ref{fig:stegen}, se muestra un circuito esquemático simplificado en el que se puede apreciar el puente auto-balanceado conectado al circuito equivalente eléctrico de la sonda XBT, representado por el resistor del termistor y dos resistores de 5 k$\Omega$ para las resistencias de los devanados que el fabricante garantiza que se encuentran precisamente apareados.  Asimismo, dentro del circuito del puente se pueden distinguir diferentes sub-componentes a saber: una fuente de corriente constante, un amplificador operacional que controla el \textit{gate} del transistor JFET Q1 y un conjunto de amplificadores operacionales para extraer una tensión proporcional al valor de resitencia del termistor sin afectar el balance ni el normal funcionamiento del puente.

Se puede observar en el circuito que se utiliza una fuente de corriente constante con el transistor Q1 polarizado para suministrar $I_S = 250\; \mu A$. Esta corriente se reparte en partes iguales por las dos ramas del puente debido a que la suma de las impedancias de cada rama está precisamente apareada por diseño. Para que esto se cumpla, se deben utilizar resistores de precisión para los resistores de 10 K$\Omega$ en la parte superior del puente, con bajo valor de dispersión respecto a su valor nominal.
...
Los amplificadores operacionales OA2, OA3 y OA4 funcionan en configuración amplificador diferencial con \textit{buffers} de entrada para no afectar el equilibrio del puente y producen una salida de tensión proporcional a la resistencia del transistor Q2.

Este diseño tiene la ventaja de producir una tensión linealmente proporcional al valor de resistencia del termistor

\begin{figure}[htpb]
	\centering
	\includegraphics[width=0.7\textwidth]{graficos/self_balancing_bridge.png}
