...
\begin{equation}
V_{out} = -V1 \cdot \left(\frac{R_3}{R_1}\right) + V_2 \cdot \left(\frac{R_4}{R_2 + R_4}\right) \cdot \left(\frac{R1 + R_3}{R_1}\right)
\end{equation}
...
\noindent donde se aprecia la naturaleza de la resta de tensión que realiza el circuito entre sus entradas $V_1$ y $V_2$ y el factor de escala $\left(\frac{R_3}{R_1}\right)$ que dan nombre a esta topología circuital.

Se ensayó un diseño que toma la tensión del terminal B de la sonda directamente de la entrada del registrador Sippican.  Este diseño agrega al amplificador diferencial un segundo amplificador operacional en configuración seguidor por emisor para evitar efectos de carga sobre el puente balanceado Sippican. Asimismo, se agregó un divisor resistivo conectado a la entrada inversora para controlar la tensión que se resta y ajustar el cero de la salida $V_{out}$.  Los valores de $R_1$ y $R_3$ se ajustan para que la salida no sature.

Se eligió un amplificador operacional de tipo \textit{rail to rail} para que la salida pueda variar en todo el rango de tensión de 0 a la tensión de alimentación, elegida en 3,3 V para que sea compatible con los niveles de tensión de las entradas del ADC de la placa NUCLEO-STM32F429ZI.  

En la FIG. \ref{fig:scaling} se muestra el circuito de la interfaz conectado al circuito puente del registrador Sippican en el punto indicado como LAUCHER B.


\begin{figure}[htpb]
\centering
\includegraphics[width=\textwidth]{graficos/Circuito_scaling.png}
\caption{Circuito eléctrico equivalente de una sonda XBT.}
\label{fig:scaling}
\end{figure}
