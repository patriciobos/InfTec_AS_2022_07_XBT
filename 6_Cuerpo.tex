...
La lógica de funcionamiento del sistema de registros batitermográficos se encuentra contenido en la unidad de registro MK 2A-1.  Esta unidad cuenta con una fuente de alimentación valvular, circuitos analógicos para acondicionar la señal proveniente de la sonda XBT y componentes electromecánicos para controlar una pluma o \textit{stylus} y el avance del papel graduado, a fin de imprimir el perfil de temperatura medido. 
...
\begin{itemize}
\item RELOAD. Es un indicador lumínico de color rojo, ubicado abajo a la derecha y es visible a través del vidrio de la puerta del registrador. Se enciende cuando el sistema está preparado para que se inserte una sonda XBT en el lanzador.
...
Las sondas batitermográficas tienen un termistor conectado a una bobina de cable. El cable se desenrolla mientras la sonda cae verticalmente en el agua a una tasa de descenso conocida. Los cambios en la resistencia del termistor debido a cambios en la temperatura del agua se transmiten al registrador a bordo a través del cable de la bobina de la sonda y luego a través del cable que conecta el lanzador con el registrador.
...
Se consideraron distintas alternativas para el diseño de la interfaz circuital o etapa de acondicionamiento de las señales de las sonda XBT. 

Una primera característica a definir fue si se iba a reemplazar el sistema de registro Sippican o no ya que el diseño eléctrico de la interfaz depende fuertemente de esta característica. La lectura del valor de temperatura del XBT depende de que se energice la sonda externamente, debido a que las sonda XBT son pasivas. Si se opta por reemplazar al sistema de registro Sippican, la interfaz que se diseñe debe proveer un mecanismo para excitar al termistor presente en la sonda.  Cabe aclarar que la excitación de la sonda es una función excluyente de uno u otro componente y que no puede ser realizada por más de un elemento en simultáneo.

La etapa de acondicionamiento o interfaz circuital actúa como enlace entre la sonda XBT y una etapa de digitalización que se implementa con un kit de desarrollo NUCLEO-STM32F429ZI, basado en un microcontrolador Cortex-M4 de ST Microelectronics.  Por este motivo, los niveles de tensión de salida de la interfaz se deben mantener en el rango 0-3,3 V para evitar posibles daños en las entradas de los conversores analógicos digitales del kit.  
...
En las siguientes subsecciones se abordan los detalles de cada diseño y se analiza su capacidad para generar una señal de tensión analógica proporcional al valor de resistencia del elemento sensor presente en las sondas XBT.
...
En modo \textit{reload} la entrada AI4 queda desconectada y, en caso de que se la vincule a tierra a través de una resistencia de valor elevado ($R_\text{d3}$ en la FIG. \ref{fig_disenio_definitivo}), registrará un estado bajo (en torno a 0 V).
...
\begin{figure}[htpb]
	...
	\caption{Circuito esquemático completo del puente auto-balanceado con componentes activos.}
...
En la FIG. \ref{fig:puenteJFET_errorRth} se muestra el error relativo en la estimación de $R_{th}$, para todo el rango de valores operativos de las sonda XBT, donde se asume que la única fuente de incerteza está en la determinación de $I_S$.  El error relativo máximo se da para $R_{th} = 3\;k\Omega$ y es igual a 0,267.
...
Se mandó a fabricar un lote de siete PCBs a la empresa Mayer Circuitos Impresos en material FR4 con máscara antisoldante en toda la superficie a excepción de los pads.  En la FIG. \ref{fig:PCB_lote} se observa el lote de placas fabricadas sin los componentes montados.  A dos de estas placas, se les montaron los componentes para la realización de ensayos. Una placa fue enviada a la Base Naval Puerto Belgrano, donde se encuentra parte del equipo de trabajo, mientras que la otra permanece en el DPA.  El objetivo es poder realizar pruebas independientes y contrastar los resultados de los ensayos.

En la FIG. \ref{fig:PCB_montado} se puede apreciar una placa con todos los componentes montados.  Cabe señalar que los dos resistores de derivación para el DAC, de 10 $k\Omega$, incluido en el circuito esquemático no se encuentran debidamente routeados en el PCB y que fueron agregados directamente entre pads del conector J9 y GND.  Esto se debe a que su necesidad fue advertida luego que los PCBs fueran fabricados y se contempla incluirlos en futuras reversiones del PCB, si las hubiera.
...
\label{tabla:ensayo}
\begin{tabular}{c|c|c|c}
Identificador & Función & Valor nominal & Valor medido \\ \hline
...
El promedio del ensamble resulta ser un estimador insesgado del promedio de los valores arrojados por la variable determinísticas en el intervalo de medición y su desvío estándar es el desvío estándar de la distribución de la componente aleatoria  disminuido en $\sqrt{N}$ \citep{degroot86}. Así, la incerteza absoluta puede decrementarse arbitrariamente a medida que se toman más mediciones y se las promedia. 
...
En el marco del proyecto para modernizar el sistema de adquisición y registro de trazos batitermográficos se completó el diseño e implementación de una etapa de acondicionamiento para señales provenientes de sonda XBT.  Para alcanzar este objetivo, primero se hizo una revisión exhaustiva del sistema Sippican actualmente en uso en los buques del COFM.  La revisión incluyó estudiar los procedimientos de operación, los principios de funcionamiento y la documentación técnica provista por el fabricante.  Asimismo, se examinaron trabajos afines disponibles en la literatura para evaluar la conveniencia de su aplicación a este proyecto.

Se avanzó en el análisis de cuatro diseños circuitales para la interfaz entre las sondas XBT y un microcontrolador, específicamente con su conversor analógico-digital.  Para esta etapa del proyecto se consideró adecuado trabajar con el kit de desarrollo NUCLEO-STM32F429ZI de la empresa ST Microelectronics, debido a su disponibilidad y al \textit{know-how} de los miembros del equipo de trabajo del proyecto.

Se evaluaron dos diseños circuitales para funcionar en simultáneo con el registrados Sippican MK 2A-1, de los cuales uno no resultó viable por encontrarse dependencia en la correcta lectura de la información de temperatura con el tipo de sonda (T-4, T-5, etc).  

Se evaluaron también dos diseños circuitales para funcionar en reemplazo del registrador Sippican MK 2A-1.  Un diseño de elaboración propia que contempla dos modos de funcionamiento para la lectura de los datos de las sonda: a lazo abierto o lazo cerrado; y que además tiene la posibilidad de simular la operación del sistema para entrenamiento del personal y/o mantenimiento de los equipos.  Este diseño fue implementado y ensayado.  Por otra parte,  el segundo diseño que reemplaza al registrador Sippican fue tomado de la literatura, y si bien fue publicado hace décadas, su principio de funcionamiento sigue vigente. 

Se implementaron dos interfaces con el diseño de elaboración propia.  Las pruebas de funcionamiento con este circuito resultaron aceptables aunque se hizo evidente, para este y todos los diseños, la necesidad de utilizar resistores de precisión para garantizar la exactitud y baja dispersión en las mediciones de temperatura.

Como trabajo futuro, se puede avanzar en la implementación de todos los diseños presentados y realizar una comparación de factibilidad y desempeño que permita identificar el más adecuado para esta aplicación.  Asimismo, se debe avanzar en la programación del \textit{firmware} para el microcontrolador y en el diseño de una estrategia de adquisición y digitalización que minimice los errores en toda la cadena de medición.
